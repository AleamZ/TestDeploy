...
const AddSale: React.FC = () => {
  ...
  const handleCreateSale = async () => {
    try {
      setIsLoading(true);
      const responseImageHeader = await uploadImage(
        imageHeader,
        "https://bluetooth-mobile-web-backend.onrender.com/upload/single"
      );
      const responseImageBanner = await uploadImage(
        imageBanner,
        "https://bluetooth-mobile-web-backend.onrender.com/upload/single"
      );
      const responseImageBackground = await uploadImage(
        imageBackground,
        "https://bluetooth-mobile-web-backend.onrender.com/upload/single"
      );
      const responseList = await uploadMultipleImages(
        listImage,
          ...
          <div className="add-event-discount-value">
            ...
            <Input
              ...
              placeholder={`Nhập giá trị giảm ${
                formData.discountType === "percentage" ? "theo %" : "theo VNĐ"
              }`}
              onChange={(e) => {
                if (formData.discountType === "percentage") {
